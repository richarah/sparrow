...
# Global env
RUN pip install -U pip setuptools wheel meson ninja && \
    ...
    pip install -r sparrow-ml/llm/requirements_unstructured.txt && \
    pip install -r sparrow-data/ocr/requirements.txt && \
    pip install llama-index-readers-file pytesseract pillow llama-index --upgrade --no-cache-dir --force-reinstall
